...
def diagnostic_snapshot(root, version, astrbot_version, runtime, status, scope, configurations, migration, secrets=()):
    ...
    return {
        ...
        'runtime': {'status': state_value(runtime.get('status', 'unknown')), 'ready': bool(runtime.get('ready')),
                    'error': sanitize_text(runtime.get('error') or (runtime.get('node') or {}).get('error')
                                           or (runtime.get('npm') or {}).get('error'), secrets),
                    'nodeError': sanitize_text((runtime.get('node') or {}).get('error'), secrets),
                    'npmError': sanitize_text((runtime.get('npm') or {}).get('error'), secrets)},
