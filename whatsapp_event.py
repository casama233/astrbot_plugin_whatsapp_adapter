...
class WhatsAppMessageEvent(AstrMessageEvent):
    ...
    async def react(self, emoji: str) -> None:
        """Record arbitration observations only after a successful Gateway send."""
        if not self.source_message_id:
            return
        try:
            await self.client.react(self.target_jid, self.source_message_id, emoji, self.source_participant)
        except Exception as exc:
            logger.warning("WhatsApp 表情回应发送失败: %s", exc)
            return
        try:
            _reaction_journal.record(chat_id=self.target_jid, message_id=self.source_message_id,
                                     sender_id=self.get_self_id(), emoji=emoji)
        except Exception as exc:
            logger.debug("WhatsApp 表情已发送，仲裁记录失败: %s", exc)
    ...
    async def get_arbiter_reaction_users(self, emoji: str) -> list[str]:
