...
from .album_caption_compat import (
    ...
    album_caption_chain as _album_caption_chain,
)
...
class WhatsAppPlatformAdapter(Platform):
    ...
    async def reload(self, platform_config):
        ...
        async with self._gateway_lifecycle_lock():
    ...
    async def convert_message(self, data: dict[str, Any]) -> AstrBotMessage | None:
        ...
        abm = _apply_group_name(abm, data, self._project_public_user_id)
        abm = _apply_sender_member_tag(abm, data)
        return _apply_album_caption_message(self, abm, data)
    ...
    async def handle_msg(self, message: AstrBotMessage):
        ...
        if self.config.get("ignore_self_messages", False):
            ...
            if sender_jid and self._is_self_mention(sender_jid, self_id, self_lid):
                ...
                return
        if self._is_reaction_only(raw):
            reaction = (raw.get("extras") or {}).get("reaction") or {}
            target_key = reaction.get("key") or {}
            try:
                _reaction_journal.record(
                    chat_id=raw.get("chatJid") or target_key.get("remoteJid") or message.session_id,
                    message_id=target_key.get("id"),
                    sender_id=getattr(message.sender, "user_id", None) or raw.get("senderJid"),
                    emoji=reaction.get("text"),
                )
            except Exception as exc:
                logger.debug("WhatsApp 表情仲裁记录失败: %s", exc)
            return
    ...
    def _message_chain(self, data: dict[str, Any], text: str) -> list[Any]:
        album = _album_caption_chain(self, data)
    ...
    def _create_gateway_process(self):
        host, port = _ensure_gateway_endpoint(self)
