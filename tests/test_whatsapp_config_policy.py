...
class WhatsAppConfigPolicyTests(unittest.TestCase):
    ...
    def test_source_integrations_and_version(self) -> None:
        ...
        main_version = re.search(r'^PLUGIN_VERSION = "([^"]+)"$', main, re.MULTILINE)
        metadata_version = re.search(r"^version: ([^\s]+)$", metadata, re.MULTILINE)
        self.assertIsNotNone(main_version)
        self.assertIsNotNone(metadata_version)
        self.assertEqual(main_version.group(1), metadata_version.group(1))
        self.assertRegex(main_version.group(1), r"^\d+\.\d+\.\d+$")
